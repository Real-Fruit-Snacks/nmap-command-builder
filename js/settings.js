...
(function () {
  ...
  function fire(key, val) {
    ...
    if (String(key).indexOf("nmap-pet") === 0) window.dispatchEvent(new Event("tw:pet"));
  }
  ...
  panel.innerHTML =
    ...
    settingRow("THEME", '<span class="seg" data-pref="nmap-theme">' +
      '<button data-val="dark">DARK</button><button data-val="light">LIGHT</button>' +
      '<button data-val="system">SYSTEM</button></span>') +
    settingRow("ACCENT", '<span class="seg accent-seg" data-pref="nmap-accent">' +
    ...
    settingRow("MODE", '<span class="seg" data-petpref="nmap-pet">' +
    ...
    settingRow("NAP", petToggle("nmap-pet-nap")) +
    settingRow("FLEE", petToggle("nmap-pet-flee")) +
    settingRow("READ", petToggle("nmap-pet-read")) +
    settingRow("TRICKS", petToggle("nmap-pet-tricks")) +
    settingRow("SPEECH", petToggle("nmap-pet-speech")) +
    "</div></div>";
  ...
  function petDefaults(key) {
    return { "nmap-pet": "float", "nmap-pet-nap": "on", "nmap-pet-flee": "on",
      "nmap-pet-read": "on", "nmap-pet-tricks": "on", "nmap-pet-speech": "off" }[key];
  }
  ...
  function refreshSeg() {
    ...
    for (var i = 0; i < segs.length; i++) {
      ...
      var cur;
      if (key === "nmap-theme") cur = TW.prefs.get("nmap-theme", "system", ["dark", "light", "system"]);
      else cur = TW.prefs.get("nmap-accent", "0", ["1", "2", "3", "4", "5"]);
  ...
  function refreshPet() {
    ...
    document.getElementById("pet-size").value = TW.prefs.get("nmap-pet-size", "28");
    document.getElementById("pet-opacity").value = TW.prefs.get("nmap-pet-opacity", "70");
  }
  ...
  panel.addEventListener("click", function (e) {
    ...
    if (key === "nmap-theme") {
      if (val === "system") { TW.prefs.remove("nmap-theme"); }
      else { TW.prefs.set("nmap-theme", val); }
      applyTheme(val === "system" ? null : val);
      refreshSeg();
      return;
    }
    if (key === "nmap-accent") {
      if (val === "0") { TW.prefs.remove("nmap-accent"); }
      else { TW.prefs.set("nmap-accent", val); }
