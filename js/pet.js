(function () {
  ...
  function readSize() {
    var s = parseInt(localStorage.getItem("nmap-pet-size"), 10);
  ...
  var COLOR_COUNT = 6;
  var petColor = 0;
  try { petColor = parseInt(localStorage.getItem("nmap-pet-color"), 10) || 0; }
  ...
  function cyclePetColor() {
    ...
    applyPetColor();
    try {
      if (petColor) localStorage.setItem("nmap-pet-color", String(petColor));
      else localStorage.removeItem("nmap-pet-color");
    } catch (e) { /* private mode */ }
  }
  ...
  function applyOpacity() {
    var o = parseInt(localStorage.getItem("nmap-pet-opacity"), 10);
    if (!(o >= 15 && o <= 100)) o = 70;
    document.documentElement.style.setProperty("--pet-base-opacity", (o / 100).toFixed(3));
  }
  function readCfg() {
    cfgNap = boolKey("nmap-pet-nap", true);
    cfgFlee = boolKey("nmap-pet-flee", true);
    cfgRead = boolKey("nmap-pet-read", true);
    cfgTricks = boolKey("nmap-pet-tricks", true);
    cfgSpeech = boolKey("nmap-pet-speech", false);
    var c = parseInt(localStorage.getItem("nmap-pet-color"), 10);
  ...
  function peekCandidates() {
    ...
    if (!note) return [];
    var els = note.querySelectorAll(".panel");
    var out = [];
  ...
  function paragraphNearCenter() {
    var note = document.querySelector("main");
    if (!note) return null;
    var ps = note.querySelectorAll(".panel");
  ...
  window.addEventListener("tw:pet", function () {
